...
RUN apt-get update && \
    ...
    wget -qO /tmp/flyway-commandline-8.0.1-linux-x64.tar.gz https://repo1.maven.org/maven2/org/flywaydb/flyway-commandline/8.0.1/flyway-commandline-8.0.1-linux-x64.tar.gz && \
    tar xfz /tmp/flyway-commandline-8.0.1-linux-x64.tar.gz -C /opt && \
    ln -s /opt/flyway-8.0.1/flyway /usr/local/bin/flyway
